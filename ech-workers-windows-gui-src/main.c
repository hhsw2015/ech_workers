...
void StartProcess() {
    ...
    // 构建 ech-workers.exe 的完整路径
    snprintf(exePath, MAX_PATH, "%sech-workers.exe", g_exeDir);
    
    if (GetFileAttributes(exePath) == INVALID_FILE_ATTRIBUTES) {
        char errMsg[512];
        snprintf(errMsg, sizeof(errMsg), "错误: 找不到 ech-workers.exe 文件!\n路径: %s\r\n", exePath);
        AppendLog(errMsg);
        return;
    }
